...
class TestNodeLinkJSON(unittest.TestCase):
    def test_empty_graph(self):
        ...
        res = retworkx.node_link_json(graph)
    ...
    def test_directed_path_graph_node_attrs(self):
        ...
        res = retworkx.node_link_json(graph, node_attrs=dict)
        expected = {
            "attrs": None,
            "directed": True,
            "links": [
    ...
    def test_directed_path_graph_edge_attr(self):
        ...
        res = retworkx.node_link_json(graph, edge_attrs=dict)
        expected = {
            "attrs": None,
            "directed": True,
            "links": [
    ...
    def test_directed_path_graph_attr(self):
        graph = retworkx.PyDiGraph(attrs="label")
        res = retworkx.node_link_json(graph, graph_attrs=lambda x: {"label": x})
        expected = {
            "attrs": {"label": "label"},
            "directed": True,
            "links": [],
    ...
    def test_invalid_path_dir(self):
        nonexistent_path = tempfile.gettempdir() + "/" + str(uuid.uuid4()) + "/graph.retworkx.json"
        graph = retworkx.PyDiGraph()
        with self.assertRaises(FileNotFoundError):
            retworkx.node_link_json(graph, path=nonexistent_path)

    def test_attr_callback_invalid_type(self):
        graph = retworkx.PyDiGraph()
        with self.assertRaises(TypeError):
            retworkx.node_link_json(graph, graph_attrs=lambda _: "attrs_field")
