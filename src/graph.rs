...
impl PyGraph {
    ...
    /// Create an empty :class:`PyGraph` with pre-allocated capacity for at least the given number
    /// of ``nodes`` and ``edges``.
    ///
    ...
    ///
    ///     import rustworkx as rx
    ///     graph = rx.PyGraph.with_capacity(4, 4)
    ///     # The graph already contains space for these items.
    ///     graph.add_nodes_from(["A", "B", "C", "D"])
    ///     graph.add_edges_from_no_data([(0, 1), (1, 2), (2, 3), (3, 0)])
    #[staticmethod]
    #[pyo3(signature=(nodes, edges, multigraph=true, attrs=None), text_signature="(nodes, edges, /, multigraph=True, attrs=None)")]
    fn with_capacity(
        py: Python,
        nodes: usize,
        edges: usize,
        ...
        PyGraph {
            graph: StablePyGraph::<Undirected>::with_capacity(nodes, edges),
            node_removed: false,
            multigraph,
            attrs: attrs.unwrap_or_else(|| py.None()),
        }
